...
if ( ! class_exists( 'PPM_ThirdParties' ) ) {
	...
	class PPM_ThirdParties {
		...
		public function init() {
			add_filter( 'ppmwp_forms_settings_page_content_tabs', array( $this, 'settings_tab' ), 20, 1 );

			$ppm = ppm_wp();

			// WooCommerce.
			if ( \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_wc_pw_reset ) ) {
				...
				add_filter( 'ppm_enable_custom_forms_array', array( $this, 'ppm_enable_wc_pw_reset_form' ), 20 );
			}

			// BuddyPress User Reg.
			if ( isset( $ppm->options->ppm_setting->enable_bp_register ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_bp_register ) ) {
			...
			if ( isset( $ppm->options->ppm_setting->enable_bp_pw_update ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_bp_pw_update ) ) {
				add_filter( 'ppm_enable_custom_forms_array', array( $this, 'ppm_enable_custom_bp_reg_form' ), 20 );
				add_action( 'bp_members_validate_user_password', array( $this, 'bp_validate_pw_change' ), 10, 4 );
			}

			// LearnDash User Reg.
			if ( isset( $ppm->options->ppm_setting->enable_ld_register ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_ld_register ) ) {
			...
			if ( isset( $ppm->options->ppm_setting->enable_um_register ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_um_register ) ) {
			...
			// Ultimate PW Update.
			if ( isset( $ppm->options->ppm_setting->enable_um_pw_update ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_um_pw_update ) ) {
				add_filter( 'ppm_enable_custom_forms_array', array( $this, 'ppm_enable_custom_um_pw_update_form' ), 20 );
				add_filter( 'ppm_enable_custom_forms_array', array( $this, 'ppm_enable_custom_um_pw_reset_form' ), 20 );
				add_action( 'um_change_password_errors_hook', array( $this, 'um_change_password_errors' ), 10, 1 );
			}

			// BBpress PW Update.
			if ( isset( $ppm->options->ppm_setting->enable_bbpress_pw_update ) && \PPMWP\Helpers\OptionsHelper::string_to_bool( $ppm->options->ppm_setting->enable_bbpress_pw_update ) ) {
						...
						<tr class="setting-heading" valign="top">
							<th scope="row">
								<label for="ppm-send-summary-email">
									<h3><?php esc_html_e( 'Third party forms', 'ppm-wp' ); ?></h3>
								</label>
							</th>
						</tr>
						<tr valign="top">
								...
								<fieldset>
									<label for="ppm-enable_wc_pw_reset">
										<input name="_ppm_options[enable_wc_pw_reset]" type="checkbox" id="ppm-enable_wc_pw_reset"
												...
												<?php esc_attr_e( 'WooCommerce password reset/update page', 'ppm-wp' ); ?>
									</label>
								</fieldset>
							</td>
						</tr>
								...
								<fieldset>
									<label for="ppm-enable_um_pw_update">
										<input name="_ppm_options[enable_um_pw_update]" type="checkbox" id="ppm-enable_um_pw_update"
												...
												<?php esc_attr_e( 'Ultimate Member password reset/update form.', 'ppm-wp' ); ?>
									</label>
								</fieldset>
							</td>
								</tr>
								...
								<fieldset>
									<label for="ppm-enable_bbpress_pw_updater">
										<input name="_ppm_options[enable_bbpress_pw_update]" type="checkbox" id="ppm-enable_bbpress_pw_update"
												...
												<?php esc_attr_e( 'BBPress password update form.', 'ppm-wp' ); ?>
									</label>
								</fieldset>
							</td>
						</tr>
		...
		public function ppm_enable_wc_pw_reset_form( $args ) {
			$new = array(
				array(
		...
		public function ppm_enable_custom_um_pw_reset_form( $args ) {
			$new = array(
				array(
					'form_selector'        => '.um-password',
					'pw_field_selector'    => '#um_field_um_password_id_user_password input',
					'form_submit_selector' => '.button',
					'elements_to_hide'     => '#pass-strength-result',
				),
			);
		...
		public function um_change_password_errors( $args ) {
			if ( isset( $_POST['user_password'] ) ) {
				...
				if ( $does_violate_rules ) {
					$error_strings = $ppm->msgs->error_strings;
					foreach ( $does_violate_rules as $violation => $value ) {
						UM()->form()->add_error( 'user_password', wp_strip_all_tags( $error_strings[ $violation ] ) );
					}
					return;
				}
			}
