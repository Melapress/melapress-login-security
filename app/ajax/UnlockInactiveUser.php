...
class UnlockInactiveUser implements AjaxInterface {
	...
	public function send_unlocked_notification_email_to_user( $user_id, $reset_password ) {
		...
		$title      = \PPM_Email_Settings::replace_email_strings( isset( $ppm->options->ppm_setting->user_unlocked_email_title ) ? $ppm->options->ppm_setting->user_unlocked_email_title : \PPM_Email_Settings::get_default_string( 'user_unlocked_email_title' ), $user_id );
		...
		if ( $reset_password ) {
			...
			$msg   = isset( $ppm->options->ppm_setting->user_unlocked_email_reset_message ) ? $ppm->options->ppm_setting->user_unlocked_email_reset_message : \PPM_Email_Settings::get_default_string( 'user_unlocked_email_reset_message' );
			$args['reset_or_continue'] = $msg . ' ' . esc_url_raw( network_site_url( "$login_page?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) ) . "\n";
		} else {
			$msg   = isset( $ppm->options->ppm_setting->user_unlocked_email_continue_message ) ? $ppm->options->ppm_setting->user_unlocked_email_continue_message : \PPM_Email_Settings::get_default_string( 'user_unlocked_email_continue_message' );
			$args['reset_or_continue'] = $msg . "\n";
		}
		
		$content   = isset( $ppm->options->ppm_setting->user_unlocked_email_body ) ? $ppm->options->ppm_setting->user_unlocked_email_body : \PPM_Email_Settings::default_message_contents( 'user_unlocked' );
